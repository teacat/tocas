...
{ 
    ...
    var closest = ts(event.target).closest('.dropdown')
    
    if(closest === null)
    {
        ts('.dropdown').removeClass('visible').addClass('hidden').addClass('animating')
                       .one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function()
        {
            ts('.dropdown').removeClass('animating')
        })
    }
    else
    {
...
ts.fn.dropdown = function(command)
{
    return this.each(function()
    {
        ts(this).on('click', function(e)
        {
            ...
            ts(this).removeClass('hidden').addClass('visible').addClass('animating').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function()
            {
                ts('.dropdown').removeClass('animating')
            })
        });
