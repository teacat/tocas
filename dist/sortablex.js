    ...
    return module = {
      ...
      get: {
        ...
        value: () => {
          var values;
          values = [];
          $this.find('[data-draggable]:not([recoverable])').each(function() {
            var value;
        ...
        draggable: {
          amount: () => {
            return $this.find(Selector.DRAGGABLE).not('[recoverable]').length;
          }
        },
        dragging: {
          element: () => {
      ...
      move: {
        ...
        placeholder: (x, y) => {
          ...
          } else if (settings.vertical === false && xoffset < rect.width / 2) {
        ...
        events: () => {
          ...
          mousemove = (event) => {
            ...
            if (settings.mode === 'clone') {
              if (module.get.pointing.$container().get() !== element) {
                return module.move.placeholder(event.clientX, event.clientY);
              }
            } else {
              if (module.get.pointing.$container().sortable('get mode') === 'clone') {

              } else if (module.get.pointing.$container().sortable('get mode') === 'pull') {
                if (module.get.pointing.$container().get() === element) {
                  return module.move.placeholder(event.clientX, event.clientY);
                }
              } else {
          ...
          ts(Selector.BODY).on('mousedown', (event) => {
            ...
            if (settings.handle !== false) {
              if (!ts(event.target).is(settings.handle)) {
                return;
              }
            }
            if (settings.mode === 'put') {
              return;
            }
          ...
          return ts(Selector.BODY).on('mouseup', (event) => {
            var con, currentDraggable, currentValue, ok, originalDraggable, originalValue, point, v, x;
            ...
            module.trigger.drop();
            if (point && con.sortable('get mode') === 'pull' && !con.contains('[confirmed]')) {
      ...
      calculate: (originalValue, amount, deny) => {
        var currentAmount, currentValue, ok;
        currentValue = module.get.value();
