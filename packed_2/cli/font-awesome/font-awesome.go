...
func path(typ string) string {
	...
	case "icon":
		return pathx.Join(r, "src/icon-map.css")
	case "icons":
...
func main() {
	...
	for _, asset := range release.Assets {
		...
		// 遞迴每個圖示。
		for k, v := range icons {
			newContent += fmt.Sprintf(".ts-icon[data-icon=\"%s\"]::before {\n", k)
			// 如果這個圖示是 Logo 商標的話，那麼就額外追加商標的樣式。
			if v.Styles[0] == "brands" {
				newContent += "    font-family: \"IconsBrands\" !important;\n"
			}
			newContent += "    content: \"\\" + v.Unicode + "\";\n"
			newContent += "}\n"
		}
