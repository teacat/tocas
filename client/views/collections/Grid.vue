...
.example .ts.grid:not(.three)
{
    ...
    {
        position: absolute;
        top: calc(0.44rem - 1px);
        bottom: calc(0.44rem - 1px);
        left: calc(0.44rem - 1px);
        right: calc(0.44rem - 1px);
        z-index: 1;
        content: '';
...
.example .ts.grid:not(.three)  .column:not(.row):not(.grid)
{
    display: flex;
...
.example .ts.grid.two
{
    > .column:not(.row):not(.grid):first-child
    {
        height: 5em;
