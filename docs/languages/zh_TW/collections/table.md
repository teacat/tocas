...
<p>表格不應用來當作網頁架構，而是用來呈現資料，如果你想要用類似的架構設計網頁，請參考格線系統。</p>
...
```html
<table class="ts [[single line]] table">
    <thead>
        <tr>
            <th>使用者名稱</th>
            <th>電子郵件信箱</th>
...
```html
<table class="ts [[stackable]] table">
    <thead>
        <tr>
            <th>文章名稱</th>
            <th>註釋</th>
...
表格中的「行」可以像是支援選擇一樣，讓滑鼠移到上面或是點擊時有效果。

```html
<table class="ts [[selectable]] table">
    <thead>
        <tr>
            <th>工作區域</th>
            <th>目前狀態</th>
...
```html
<table class="ts [[top attached]] table">
    <tbody>
        <tr>
            <td>Sciuridae Li</td>
            <td>Mac Taylor</td>
...
表格中欄位的文字水平對齊也可以被改變，例如靠左對齊、右，甚至置中。
...
欄位可以只占欄位內容需要的空間，而不是自動增寬留下過多的空白。
...
```html
<table class="ts [[collapsing]] table">
    <thead>
        <tr>
            <th>#</th>
            <th>姓名</th>
...
表格的內距可以稍微縮減用以呈現更多的行數。

```html
<table class="ts [[compact]] table">
    <thead>
        <tr>
            <th>檔案名稱</th>
            <th>分類</th>
...
```html
<table class="ts [[mini]] celled table">
    <tbody>
        <tr>
            <td>mini</td>
            <td>迷你</td>
