...
表單也具有網格系統，對於這部分並不熟悉的使用者還請參閱網格系統的教學。這個元件的設計很自由化，因此使用方式可能對初心者來說過度複雜，可以參考實際範例中的註冊頁面。
...
欄位是組成表單的最基本元素，而其中可以帶有標籤用以命名該欄位。
...
但文字訊息會隨著欄位狀態而變更顏色。且比起提醒，文字訊息也稍微大了一點，可用做錯誤、警告提示。
...
多個欄位可以被規劃成群組，並自動均分寬度，在行動裝置上將會縮成一行。
