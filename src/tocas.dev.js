...
var Tocas, animationEnd, bindModalButtons, closeModal, contractDropdown, detectDropdown, expandDropdown, quadrant;
...
expandDropdown = function(target) {
  return ts(target).css('z-index', '12').removeClass('hidden').addClass('visible').addClass('animating').one(animationEnd, function() {
...
contractDropdown = function(target) {
  return ts(target).css('z-index', '11').removeClass('visible').addClass('hidden').addClass('animating').one(animationEnd, function() {
    return ts(target).removeClass('animating');
  });
};
