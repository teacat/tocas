        ...
        calculate:
            direction: =>
                ...
                switch settings.position
                    when Position.TOP
                        direction = Position.TOP if topOK
                    when Position.RIGHT
                        direction = Position.RIGHT if rightOK
                    when Position.BOTTOM
                        direction = Position.BOTTOM if bottomOK
                    when Position.LEFT
                        direction = Position.LEFT if leftOK
                if direction is settings.position
                    return direction
                switch
                    when topOK
                    ...
                    switch direction
                        when Position.TOP, Position.BOTTOM
                            ...
                            # 否則。
                                ...
                                console.log "B"
                                # 如果彈出式訊息突出限界。
                                if ((distance.inBoundary.left + popupRect.width) - distance.boundary.width < 0 and distance.inBoundary.left < distance.boundary.width / 2) or ((distance.inBoundary.left + rect.width) - popupRect.width - distance.boundary.width < 0 and distance.inBoundary.left > distance.boundary.width / 2)
                                    console.log "C"
                                    # 如果按鈕在左半邊。
