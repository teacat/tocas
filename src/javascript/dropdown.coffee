    ...
    module =
        ...
        hide: =>
            ...
            if module.trigger.hide() is false
                return
            module.set.direction()
            module.animate.contract =>
                if not module.is.hidden()
                    return
                $this.removeClass "#{ClassName.ANIMATING} #{ClassName.HIDDEN} #{ClassName.UPWARD} #{ClassName.DOWNWARD} #{ClassName.LEFTWARD} #{ClassName.RIGHTWARD}"
        ...
        listener:
            body: (target) =>
                ...
                isSelfChild = $this.is(target) or $this.contains target
        ...
        trigger:
            select: (event) =>
                ...
                settings.onSelect.call element, value, item
            show: =>
        ...
        bind:
            events: =>
                $body.on "#{Event.CLICK}#{id}", (event) =>
                    ...
                    module.listener.body event.target

                $this.on Event.CLICK, (event) =>
                    #debug "發生 CLICK 事件", element
                    module.toggle()

                $this.on Event.CLICK, Selector.ITEM, (event) =>
                    #debug "發生 CLICK 事件", element
                    module.trigger.select event
