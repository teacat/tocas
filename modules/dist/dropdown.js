  ...
  class Dropdown {
    ...
    addLabel(text, value) {
      ...
      if (label !== false && label instanceof HTMLElement) {
        return this.$this.find(':scope > .labels').append(label);
      }
    }

    removeLabel() {
      var labels;
      labels = this.$this.find(':scope > .labels > .ts.label');
      if (labels.length > 0) {
      ...
      this.$this.on('click', (event) => {
        ...
        switch (false) {
          ...
          case !(targetSelection || multipleParent || labelsParent || !isCloseButton):
            ...
            return (ref = this.$this.find('input.search').get()) != null ? ref.focus() : void 0;
