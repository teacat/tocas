...
TocasAccordion = (function() {
  class TocasAccordion {
    $init({$this, $module, $data, $delay, $options}) {
      ...
      $this.find($data.title).on('click', function() {
        ...
        // 當手風琴是開的，則關，反之亦然。
        return $module.prototype._toggle({$this, $delay}, $selector(this));
      });
      return ts.fn;
    }
    ...
    _toggle({$this, $delay}, title) {
      if (title.hasClass('active')) {
        return this._close({$this, $delay}, title);
      } else {
        return this._open({$this, $delay}, title);
      }
    }
  ...
  TocasAccordion.prototype.$methods = {
    // 開啟指定索引的手風琴內容。
    open: function({$this, $module, $delay, $data}, index) {
      $module.prototype._open({$this, $delay}, $this.find($data.title).eq(index));
      return ts.fn;
    },
    // 關閉指定索引的手風琴內容。
    close: function({$this, $module, $delay, $data}, index) {
      $module.prototype._close({$this, $delay}, $this.find($data.title).eq(index));
      return ts.fn;
    },
    // 切換指定索引的手風琴內容，如果是開啟的則關閉，相反之。
    toggle: function({$this, $module, $delay, $data}, index) {
      $module.prototype._toggle({$this, $delay}, $this.find($data.title).eq(index));
      return ts.fn;
    }
  };
