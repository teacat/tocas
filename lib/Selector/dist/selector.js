  ...
  value: function(options) {
    ...
    return this.each(function() {
      ...
      timer = () => {
        ...
        // 就返回而不要繼續執行。
        if (this.$timers[options.name].passed < interval) {
          return;
        }
        // 呼叫回呼函式。
        callback();
        ...
        clearInterval(this.$timers[options.name].timer);
        // 移除在 DOM 元素內的這個計時器物件。
        return delete this.$timers[options.name];
      };
      // 在此元素內初始化計時器物件。
...
$selector.fn.setTimeout = {
  value: function(name, callback, interval) {
    return this.each(function() {
      var timer;
      if (this.$timers === void 0) {
        this.$timers = [];
      }
      if (this.$timers[name] !== void 0) {
        clearInterval(this.$timers[name].timer);
      }
      timer = () => {
        // 替計時器加上 10 毫秒。
        this.$timers[name].passed += 10;
        // 如果計時器的經過時間還不到使用者設定的時間
        // 就返回而不要繼續執行。
        if (this.$timers[name].passed < interval) {
          return;
        }
        // 呼叫回呼函式。
        callback();
  ...
  value: function(name, callback, interval) {
    return this.each(function() {
      ...
      timer = () => {
        ...
        callback();
        // 重設計數計時器。
        return this.$timers[name].passed = 0;
      };
      // 在此元素內初始化計時器物件。
      return this.$timers[name] = {
        timer: setInterval(timer, 10),
