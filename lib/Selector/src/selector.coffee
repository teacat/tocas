...
$selector.fn.collectSwap = (callback) ->
    collection = []
...
$selector.fn.prependTo = (selector) ->
    @each ->
...
$selector.fn.hasClass = (name) ->
    @get(0)?.classList.contains(name)

# CSS
#
# 將選擇器元素套用指定的 CSS 樣式。
$selector.fn.css = (name, value) ->
...
#
# 綁定並註冊一個事件監聽器。
$selector.fn.on = (events, handler, options) ->
...
#
# 註銷事件監聽器。
$selector.fn.off = (events, handler) ->
    @each ->
        events.split(' ').forEach (eventName) ->
